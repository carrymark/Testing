...
$user_data = getRowDataByField(DBTableName::Users, "email", $thread_data["email"]);
...
?>
<div><a href="<?php echo BASE_URL?>stored_emails.php?user=<?php echo $user_data["id"];?>">Back</a></div>
<message>
...
if((int)$attCountData['attCount'] > 0){
	...
	foreach($attRows as $attRow){
		...
		if(in_array($parted[count($parted) - 1], array("png", "jpg", "jpeg", "gif", "bmp"))){
			?>
			<a target="_blank" href="<?php echo BASE_URL.'attachments/'.$attRow["message_id"]."_".$attRow["file_name"];?>"><img width="100" src="<?php echo BASE_URL.'attachments/'.$attRow["message_id"]."_".$attRow["file_name"];?>"/></a><br/>
			<?php
		}
		else
		{
	?>
		<a target="_blank" href="<?php echo BASE_URL.'attachments/'.$attRow["message_id"]."_".$attRow["file_name"];?>"><?php echo $attRow["file_name"];?></a><br/>
	<?php
		}	
	}
}
