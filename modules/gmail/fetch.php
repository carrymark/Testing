...
if(strtolower($_GET["action"]) == "new" && (int)$_GET["user"] != 0){
	...
	//echo "THREADS<pre>";print_r($threads);echo "</pre><br><hr><br>";
	//exit;
	...
	foreach($threads as $thread){
		...
		//echo "MESSAGES<pre>";print_r($thread_data);echo "</pre><br><hr><br>";
		...
		foreach($messages as $msg){
			...
			//echo "MESSAGE<pre>";print_r($message);echo "</pre><br><hr><br>";
			...
			foreach($headers as $header){
				if(strtolower($header->name) == "date") $email_date = $header->value;
				if(strtolower($header->name) == "subject") $subject = $header->value;
				if(strtolower($header->name) == "from") 
				{
					$fromIndex = $index;
					$from = $header->value;
				}
				if(strtolower($header->name) == "to") 
				{
					$toIndex = $index;
					$to = $header->value;
				}
			...
			foreach($attachments as $attachment){
				...
				$output_file = DOCROOT."attachments/".$msg->getId()."_".$attachment['filename'];
...
echo "<meta http-equiv='refresh' content='0;url=".BASE_URL."stored_emails.php?user=".(int)$_GET['user']."'>";
?>